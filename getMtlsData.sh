...
getProxiesData()
{
    ...
    # '******* Get all deployed proxies in an Env *******'
    countProxies=0
    opdkProxies=$(curl -H "Authorization: Basic $auth" -s -X GET "$msUrl/v1/organizations/$org/apis")
    countTotal=$(curl -H "Authorization: Basic $auth" -s -X GET "$msUrl/v1/organizations/$org/apis" | jq length)

    # '******* Get proxy details *******'
    for opdkProxy in $(echo "$opdkProxies" | jq -r '.[]' | sort); do
        opdkRev=0
        opdkDep="N"
        opdkSrc="-"
        opdkSrcName="-"
        ...
        url=$(echo $msUrl/v1/organizations/$org/apis/$opdkProxy/deployments | sed "s/ /%20/g")
        statusCodeExport=$(curl -H "Authorization: Basic $auth" -sS -I GET "$url" 2> /dev/null | head -n 1 | cut -d ' ' -f2)
        if [[ $statusCodeExport != 200 ]] ; then
            echo INFO: "Proxy: $opdkProxy | Revision:" 0
            csv $org $env $opdkProxy $opdkRev $opdkDep $opdkSrc $opdkSrcName $opdkPxKeystore $opdkPxKeyAlias $opdkPxTruststore >> $reportFile
        else
            proxyDeployments=$(curl -H "Authorization: Basic $auth" -s -X GET "$url")
            for proxyDeploymentsEnv in $(echo $proxyDeployments | jq -r '.environment | .[]' | jq '[.name + "," + .revision[].name] | max'); do
                opdkEnv=$(echo $proxyDeploymentsEnv | tr -d '"' | cut -d ',' -f1)
                opdkRev=$(echo $proxyDeploymentsEnv | tr -d '"' | cut -d ',' -f2)
                opdkDep="Y"
                if [[ $env == $opdkEnv ]]; then
                    ...
                    for proxyPolicy in $(echo "$proxyPolicies" | jq -r '.[]' | sort); do
                        ...
                        proxyPolicies=$(curl -H "Authorization: Basic $auth" -s -X GET "$purl/policies")
                        opdkPolType=$(echo $proxyPolicies | jq -r '.proxyPolicies')
                        if [[ $opdkPolType == "ServiceCallout" ]]; then
                            opdkSrc=$opdkPolType
                            opdkTsEnabled=$(echo $opdkTargetServerDetails | jq -r '.isEnabled')
                            [ -z "$opdkTsEnabled" ] && opdkTsEnabled="-"
                            opdkPolSslClientAuthEnabled=$(echo $proxyPolicies | jq -r '.targetConnection.sSLInfo.clientAuthEnabled')
                            [ -z "$opdkTsSslClientAuthEnabled" ] && opdkTsSslClientAuthEnabled="-"
                            opdkTsSslKeystore=$(echo $proxyPolicies | jq -r '.sSLInfo.keyStore')
                            [ -z "$opdkTsSslKeystore" ] && opdkTsSslKeystore="-"
                            opdkTsSslKeyalias=$(echo $proxyPolicies | jq -r '.sSLInfo.keyAlias')
                            [ -z "$opdkTsSslKeyalias" ] && opdkTsSslKeyalias="-"
                            opdkTsSslTruststore=$(echo $proxyPolicies | jq -r '.sSLInfo.trustStore')
                            [ -z "$opdkTsSslTruststore" ] && opdkTsSslTruststore="-"
                        fi
                    done
                    #header=ORGANIZATION,ENVIRONMENT,PROXY,REVISION,DEPLOYED,SOURCE,SOURCENAME,KEYSTORE,KEYALIAS,TRUSTSTORE
        ...
        countProxies=$((countProxies+1))
    done
    echo "***** Total Deployed: $countProxies/$countTotal"
}
